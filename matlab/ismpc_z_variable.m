...
fs_timing = [0; 50; 100; 150; 200; 250; 300; 350; 400; 450; 500; 550; 600; 650; 700];
...
eta = sqrt(9.81/z0);
eta =@(xz,ti,tf) sqrt(9.81/z0);
dsSamples = 20; %Double support samples 20 per 50
% fs_timing = fs_timing./2;
C = 50;
P = 100;
...
for j = 1:simDuration
    ...
    b_zmpconstr_cam = [pzmp*(xz_cam_max-xz_cam); ...
        pzmp*(yz_cam_max-yz_cam); ...
        -pzmp*(-xz_cam_max-xz_cam); ...
        -pzmp*(-yz_cam_max-yz_cam)];
...
%         y_cam + yd_cam/eta - yz_cam ];
    ...
    b_stabconstr_cam = [x_cam + xd_cam/eta - xz_cam ; ...
        y_cam + yd_cam/eta - yz_cam];
    %
    %     A_stabconstr_cam = zeros(2,2*C);
    %     b_stabconstr_cam = zeros(2,1);
    ...
    decisionVariables = quadprog(H, f, [A_zmpconstrwithcam; A_kinconstrwithcam], [b_zmpconstrwithcam; b_kinconstr], ...
        [A_stabconstrwithcam; A_termconstrwithcam], [b_stabconstrwithcam; b_termconstr_cam],...
        [],[],[],options);
...
%     yz = yz + yz_cam;
    ...
    % store trajectories
    x_store(end+1) = x + x_cam ;
    xz_store(end+1) = xz + xz_cam;
    
    y_store(end+1) = y + y_cam;
    yz_store(end+1) = yz + yz_cam;    
