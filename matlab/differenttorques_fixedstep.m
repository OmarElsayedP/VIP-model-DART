...
%     Tzdes = zeros(size(Tydes));
    ...
    if(VIP)
        ...
        if(~Angle_const)
            A_angleconstrfull = [zeros(size(A_angleconstr)), zeros(size(A_angleconstr))];
            b_angleconstr = zeros(size(b_angleconstr));
        else
        ...
        A_angleconstrfull = zeros(1,2*size(A_angleconstr,2));
        b_angleconstr = 0;
    end
    Qzdot = 1;
    Qzdot_cam = 10;
