...
inline Eigen::VectorXd solveQP_hpipm_cam(Eigen::MatrixXd costFunctionH, Eigen::VectorXd costFunctionF, 
        Eigen::MatrixXd AConstraint, Eigen::VectorXd bConstraintMin, Eigen::VectorXd bConstraintMax) {

    int n_variables = costFunctionH.cols();
    int n_constr = AConstraint.rows() - 4;

    int nv = n_variables;
    int ne = 4;  //number of equality constraints
    //if (itr == 20 ) ne = 0;
    ...
    double A[2*n_variables] = {};
    double b[2] = {};
    // std::cout << "H_size= " << sizeof(H) << std::endl;
    // std::cout << "H_size should be = " << n_variables*n_variables << std::endl;
    ...
    for (int i = 0; i<n_variables; i++) {
        ...
        for (int j = 0; j<n_variables; j++) {
            ...
            // std::cout << "j*n_variables+i = " << j*n_variables+i;
                // std::cout << "i = " << i;
    ...
    for (int k = 0; k<n_constr; k++) {
        d_lb[k] = bConstraintMin(k+4); //bConstraintMin(k);
        d_ub[k] = bConstraintMax(k+4); //bConstraintMax(k);

        for (int j = 0; j<n_variables; j++) {
            C[j*n_constr+k] = AConstraint(k+4,j); //AConstraint(k,j);
        }
    }
    for (int k = 0; k<2; k++) {
        b[k] = bConstraintMax(k); //beq(k);
