...
void FootstepPlan::plan(std::vector<Vref> vrefSequence, Eigen::VectorXd initialLeftFoot, Eigen::VectorXd initialRightFoot, bool firstSupportFootIsLeft) {
    ...
    // std::cout << "samplesPerStep" << samplesPerStep << std::endl;
        for (int i = 0; i < nFootsteps; i++) {
            ...
            if (i==1) tempFootstep << 0.0, -FootStepY, 0.0, 0.0, 0.0, 0.0, i*samplesPerStep;
            // if(i>1) tempFootstep << (i-1)*0.0, pow(-1,i)*FootStepY, 0.0, 0.0, 0.0, 0.0, i*samplesPerStep;

            if(i>1) tempFootstep << (i-1)*0.15, pow(-1,i)*FootStepY, 0.0, 0.0, 0.0, 0.0, i*samplesPerStep;

            // if(i>1) tempFootstep << (i-1)*0.0, pow(-1,i)*0.075, 0.0, 0.0, 0.0, 0.0, i*samplesPerStep;
